...
*/
/**
 * Administración de la tabla group
*
 * Esta clase se encarga de la administración de la tabla group brindando métodos que permiten insertar, modificar, eliminar o mostrar registros de la misma.
*
* Ejemplo:
* <code>
* <?php
...
class Cgroup extends Cbase
{
    ...
    * - Tabla: {@link Cgroupxpermission groupxpermission}
    * - Campo: {@link Cgroupxpermission::$idGroup idGroup}
    ...
    */
	...
	require_once (CLASS_LANGUAGE_PATH.CLASS_LANGUAGE.FILE_SLASH.'Cgroup.php');
    }
    ...
    */
	...
	{
	    $this->id = $id;
	    $this->addError(CGROUP_SETID_REQUIRED_VALUE);
	...
	else
	{
	    ...
	    {
		$this->addError(CGROUP_SETID_VALIDATE_VALUE);
	...
	{
	    $this->name = $name;
	    $this->addError(CGROUP_SETNAME_REQUIRED_VALUE);
    ...
    */
    ...
    /**
    * Inserta un nuevo registro en la tabla group
    *
    ...
    */
	...
	{
	    $this->addError(CGROUP_ADD_ERROR);
	    return FALSE;
	}
	else
	{
    ...
    /**
    * Actualiza un registro de la tabla group
    *
    ...
    */
	...
	if (validateRequiredValue($this->id) === TRUE)
	{
	    ...
	    if ($this->getDbConn()->Execute($sql) === FALSE)
	    {
		$this->addError(CGROUP_UPDATE_ERROR);
		return FALSE;
	    }
	    else
	    {
	...
	{
	    $this->addError(CGROUP_UPDATE_REQUIRED_PK);
	    return FALSE;
	}
    }

    /**
    * Elimina un registro de la tabla group
    *
    ...
    */
	...
	if (validateRequiredValue($this->id) === TRUE)
	{
	    ...
	    if ($this->getDbConn()->Execute($sql) === FALSE)
	    {
		$this->addError(CGROUP_DEL_ERROR);
		return FALSE;
	    }
	    else
	    {
	...
	{
	    $this->addError(CGROUP_DEL_REQUIRED_PK);
	    return FALSE;
	}
    }

    /**
    * Obtiene un registro de la tabla group
    *
    ...
    */
	...
	if (validateRequiredValue($this->id) === TRUE)
	{
	    ...
	    {
		$this->addError(CGROUP_GETDATA_ERROR);
		return FALSE;
	    }
	}
	else
	{
	    $this->addError(CGROUP_GETDATA_REQUIRED_PK);
	    return FALSE;
	}
    }

    /**
    * Obtiene un conjunto de registros de la tabla group
    *
    ...
    */
	...
	if ($rs !== FALSE)
	{
	    ...
	    if ($rs === FALSE)
	    {
		$this->addError(CGROUP_GETLIST_ERROR);
		return FALSE;
	    }
	    else
	    {
	...
	{
	    $this->addError(CGROUP_GETLIST_TOTAL_LIST_ERROR);
	    return FALSE;
	}
    }

    /**
    * Verifica si ya existe en la tabla group el valor Nombre seteado
    *
    * Este método controla si ya existe en la tabla group un registro con el valor {@link $name Nombre} seteado.
    ...
    * Si no está seteado el valor {@link $name Nombre} el método devuelve FALSE.
    *
    * @param boolean $update [opcional] indica si el método se está llamando durante la actualización de un registro
    * @return boolean
    * @access public
    */
    public function existName($update = FALSE)
    {
	if (validateRequiredValue($this->name) === TRUE)
	{
		...
		if (count($row) > 0)
		{
		    $this->addError(CGROUP_EXIST_NAME_EXIST);
		    return TRUE;
		}
		else
		{
	    ...
	    {
		$this->addError(CGROUP_EXIST_NAME_ERROR);
		return TRUE;
	    }
	}
	else
    ...
    /**
     * Devuelve el último valor ID insertado en la tabla group
     * 
	...
	if ($row === FALSE)
	{
	    $this->addError(CGROUP_GET_LAST_ID_ERROR);
	    return FALSE;
	}
	else
	{
    ...
    /**
    * Muestra un formulario para dar de alta un registro de la tabla group
    *
    * Este método muestra un formulario para dar de alta un registro de la tabla group mostrando sólo los campos seteados en el parámetro $fields.
    *
    ...
    */
	...
	if ($_POST['addGroup'] == 'add')
	{
		...
		<div class="form add">
		    ...
		    <div class="top"></div>
		    <div class="fields">
			    <div class="message success"><?php echo CGROUP_ADD_FORM_OK; ?></div>
		    </div>
		    ...
		    {
		    ?>
			    <input type="button" value="<?php echo CGROUP_ADD_FORM_OK_BTN; ?>" onclick="location.href='<?php echo $href; ?>'" class="success" />
		    <?php
		    }
		    ?>
		    </div>
		...
		<div class="form add">
		    ...
		    <div class="top"></div>
		    <form name="formAddGroup" id="formAddGroup" method="post" action="">
		    <input name="addGroup" type="hidden" id="addGroup" value="back" />
		    <div class="fields">
		    <?php
		    if (in_array('name', $arrayFields) === TRUE)
		    {
		    ...
		    <div class="middle"></div>
		    <div class="buttons">
			    <input type="submit" value="<?php echo CGROUP_ADD_FORM_BACK_BTN; ?>" class="back" />
		    </div>
		    </form>
		    <div class="bottom"></div>
		</div>
	...
	else
	{
		...
		<div class="top"></div>
		<form name="formAddGroup" id="formAddGroup" method="post" action="">
		    <input name="addGroup" type="hidden" id="addGroup" value="add" />
		    <div class="fields">
		    <?php
			...
			if ($value == 'name')
			{
			    ?>
			    <div class="field">
				<label><?php echo CGROUP_ADD_FORM_LABEL_FIELD_NAME; ?> <span>*</span></label>
		    ...
		    <div class="middle"></div>
		    <div class="buttons">
			<input type="submit" value="<?php echo CGROUP_ADD_FORM_SUBMIT_BTN; ?>" class="accept" />
			<?php
			if (validateRequiredValue($href) === TRUE)
			{
			    ?>
			    <input type="button" value="<?php echo CGROUP_ADD_FORM_CANCEL_BTN; ?>" onclick="location.href='<?php echo $href; ?>'" class="cancel" />
			    <?php
			}
			?>
		    </div>
		...
		<div class="bottom"></div>
		<span class="required">* <?php echo CGROUP_ADD_FORM_LABEL_REQUIRED; ?></span>
	    </div>
	    <?php
	}
    }
    ...
    /**
    * Muestra un formulario para actualizar un registro existente de la tabla group
    *
    * Este método muestra un formulario para actualizar un registro de la tabla group mostrando sólo los campos seteados en el parámetro $fields.
    ...
    */
	...
	if (isset($_POST['updateGroup']) === FALSE)
	{
	    $_POST['updateGroup'] = '';
	}
	...
	if ($_POST['updateGroup'] == 'update')
	{
		...
		<div class="form update">
		    ...
		    <div class="top"></div>
		    <div class="fields">
			<div class="message success"><?php echo CGROUP_UPDATE_FORM_OK; ?></div>
		    </div>
		    <div class="middle"></div>
		    <div class="buttons">
		    <?php
		    ...
		    {
		    ?>
			    <input type="button" value="<?php echo CGROUP_UPDATE_FORM_OK_BTN; ?>" onclick="location.href='<?php echo $href.($_GET['p'] != '' ? '?p='.$_GET['p'] : ''); ?>'" class="success" />
		    <?php
		    }
		    ?>
		    </div>
		...
		<div class="form update">
		    ...
		    <div class="top"></div>
		    <form name="formUpdateGroup" id="formUpdateGroup" method="post" action="">
		    <input name="updateGroup" type="hidden" id="updateGroup" value="back" />
		    <div class="fields">
			<?php
			if (in_array('name', $arrayFields) === TRUE)
			{
		    ...
		    <div class="middle"></div>
		    <div class="buttons">
			<input type="submit" value="<?php echo CGROUP_UPDATE_FORM_BACK_BTN; ?>" class="back" />
		    </div>
		    </form>
		    <div class="bottom"></div>
		</div>
	...
	else
	{
		...
		<div class="form update">
		    ...
		    <div class="top"></div>
		    <form name="formUpdateGroup" id="formUpdateGroup" method="post" action="">
			<input name="updateGroup" type="hidden" id="updateGroup" value="update" />
			...
			foreach ($arrayFields as $value)
			{
				...
				?>
				<div class="field">
				    <label><?php echo CGROUP_UPDATE_FORM_LABEL_FIELD_ID; ?></label>
				...
				?>
				<div class="field">
				    <label><?php echo CGROUP_UPDATE_FORM_LABEL_FIELD_NAME; ?> <span>*</span></label>
				    <input name="name" type="text" id="name" value="<?php echo $this->getName(); ?>" class="str" maxlength="50" />
				</div>
				<?php
			    }
			...
			<div class="middle"></div>
			<div class="buttons">
			    <input type="submit" value="<?php echo CGROUP_UPDATE_FORM_SUBMIT_BTN; ?>" class="accept" />
			    <?php
			    if (validateRequiredValue($href) === TRUE)
			    {
			    ?>
				    <input type="button" value="<?php echo CGROUP_UPDATE_FORM_CANCEL_BTN; ?>" onclick="location.href='<?php echo $href.($_GET['p'] != '' ? '?p='.$_GET['p'] : ''); ?>'" class="cancel" />
			    <?php
			    }
			    ?>
			</div>
		    ...
		    <div class="bottom"></div>
		    <span class="required">* <?php echo CGROUP_UPDATE_FORM_LABEL_REQUIRED; ?></span>
		</div>
		<?php
	    }
	    else
		...
		<div class="form update">
		    ...
		    {
		    ?>
			    <input type="button" value="<?php echo CGROUP_UPDATE_FORM_BACK_BTN; ?>" onclick="location.href='<?php echo $href.($_GET['p'] != '' ? '?p='.$_GET['p'] : ''); ?>'" class="back" />
		    <?php
		    }
		    ?>
		    </div>
    ...
    /**
    * Elimina un registro existente de la tabla group y muestra el resultado obtenido
    *
    * Este método intenta eliminar un registro de la tabla group y muestra el resultado obtenido, redireccionando o no a la página correspondiente de acuerdo a los parámetros seteados.
    ...
    */
	...
	?>
		...
		if ($this->error() === FALSE)
		{
		    ?>
		    <div class="fields">
			<div class="message success"><?php echo CGROUP_DEL_FORM_OK; ?></div>
		    </div>
		    <div class="middle"></div>
		    <div class="buttons">
			<?php
			...
			{
			?>
			    <input type="button" value="<?php echo CGROUP_DEL_FORM_OK_BTN; ?>" onclick="location.href='<?php echo $href.($_GET['p'] != '' ? '?p='.$_GET['p'] : ''); ?>'" class="success" />
			<?php
			}
			?>
		    </div>
		...
		else
		{
		    ...
		    <div class="middle"></div>
		    <div class="buttons">
			<input type="button" value="<?php echo CGROUP_DEL_FORM_BACK_BTN; ?>" onclick="location.href='<?php echo $href.($_GET['p'] != '' ? '?p='.$_GET['p'] : ''); ?>'" class="back" />
		    </div>
		    <?php
		}
		?>
    ...
    /**
    * Elimina un grupo de registros existente de la tabla group y muestra el resultado obtenido
    *
    * Este método intenta eliminar un grupo de registros de la tabla group y muestra el resultado obtenido, redireccionando o no a la página correspondiente de acuerdo a los parámetros seteados.
    *
    ...
    */
	...
	{
	    $this->addError(CGROUP_DEL_GROUP_FORM_REQUIRED_PK);
	}
	...
	<div class="form del-group">
		...
		?>
		<div class="fields">
		    <div class="message success"><?php echo CGROUP_DEL_GROUP_FORM_OK; ?></div>
		</div>
		<div class="middle"></div>
		<div class="buttons">
		    <?php
		    ...
		    {
			?>
			<input type="button" value="<?php echo CGROUP_DEL_GROUP_FORM_OK_BTN; ?>" onclick="location.href='<?php echo $href.($_POST['p'] != '' ? '?p='.$_POST['p'] : ''); ?>'" class="success" />
			<?php
		    }
		    ?>
		</div>
		...
		{
		    $this->deleteErrors();
		    $this->addError(CGROUP_DEL_GROUP_FORM_CANT_DELETE_ALL);
		}
		?>
		<div class="fields">
		    <div class="message error"><?php $this->showErrors(); ?></div>
		...
		<div class="middle"></div>
		<div class="buttons">
		    <input type="button" value="<?php echo CGROUP_DEL_GROUP_FORM_BACK_BTN; ?>" onclick="location.href='<?php echo $href.($_POST['p'] != '' ? '?p='.$_POST['p'] : ''); ?>'" class="back" />
		</div>
		<?php
	    }
	    ?>
    ...
    */
	...
	<div class="form show">
		...
		if ($value == 'id')
		{
		    ?>
		    <div class="field">
			<label><?php echo CGROUP_SHOW_DATA_LABEL_FIELD_ID; ?></label>
			<strong class="pk"><?php echo $this->getId(); ?></strong>
		    </div>
		    <?php
		}
		if ($value == 'name')
		{
		    ?>
		    <div class="field">
			<label><?php echo CGROUP_SHOW_DATA_LABEL_FIELD_NAME; ?></label>
	    ...
	    {
		?>
		<input type="button" value="<?php echo CGROUP_SHOW_DATA_BACK_BTN; ?>" onclick="location.href='<?php echo $href.($_GET['p'] != '' ? '?p='.$_GET['p'] : ''); ?>'" class="back" />
		<?php
	    }
	    ?>
	    </div>
    ...
    /**
    * Muestra un formulario de búsqueda de registros de la tabla group
    *
    * Este método muestra un formulario con los campos seteados en el parámetro $fields para realizar una búsqueda de los registros de la tabla group.
    *
    ...
    */
	...
	<div class="form search">
	    ...
	    if ($showHideBtn === TRUE)
	    {
		if (isset($_SESSION['main_tr_search_group']) === FALSE)
		{
		    $_SESSION['main_tr_search_group'] = '';
		}

		$display  = 'display: none;';
		$btnClass = 'closed';
		if ($_SESSION['main_tr_search_group'] === 'open')
		{
		    $display  = '';
		    $btnClass = 'open';
		}
	    ...
	    ?>
	    <div class="title">
		<div class="ico"><?php if ($showHideBtn === TRUE) { ?><a href="#" onclick="showHideSearch('group'); return false;" class="<?php echo $btnClass; ?>"></a><?php } ?></div>
		<div class="label"><a href="#" onclick="showHideSearch('group'); return false;"><?php echo $title; ?></a></div>
	    </div>
	    <div id="container_search_group" style="<?php echo $display; ?>">
		<div class="top"></div>
		<form name="formSearchGroup" id="formSearchGroup" method="<?php echo $method; ?>" action="">
		<div class="fields">
		<?php
		$condition = array();
		$params = array();
		...
		foreach ($arrayFields as $value)
		{
			...
			?>
			<div class="field">
			    <label><?php echo CGROUP_SEARCH_FORM_LABEL_FIELD_ID; ?></label>
			...
			?>
			<div class="field">
			    <label><?php echo CGROUP_SEARCH_FORM_LABEL_FIELD_NAME; ?></label>
		...
		<div class="middle"></div>
		<div class="buttons">
		    <input type="submit" value="<?php echo CGROUP_SEARCH_FORM_SUBMIT_BTN; ?>" class="search" />
		</div>
		</form>
		<div class="bottom"></div>
	    </div>
    ...
    /**
    * Muestra el resultado de una consulta a la tabla group
    *
    * Este método muestra un conjunto de registros que son el resultado de una consulta a la tabla group. Muestra sólo los campos seteados en el parámetro $fields.
    *
    ...
    * - 'class'   : clase css
    * Nota: Los registros seleccionados se envían al archivo seteado por medio del método post en la siguiente variable: <b>$_POST['groupGroup'] (array)</b>
    *
    ...
    */
	...
	foreach ($arrayField as $value)
	{
		...
		$head.= '<div class="col" style="width: '.$arrayWidth['id'].';"><div class="num"><a href="'.$href.'">'.altText(getCutString(CGROUP_SHOW_QUERY_HEAD_FIELD_ID, $arrayStrLen['id']), CGROUP_SHOW_QUERY_HEAD_FIELD_ID).'</a></div></div>';
		$headers['id'] = '<div class="num"><a href="'.$href.'">'.altText(getCutString(CGROUP_SHOW_QUERY_HEAD_FIELD_ID, $arrayStrLen['id']), CGROUP_SHOW_QUERY_HEAD_FIELD_ID).'</a></div>';
	    }
		...
		$head.= '<div class="col" style="width: '.$arrayWidth['name'].';"><div class="str"><a href="'.$href.'">'.altText(getCutString(CGROUP_SHOW_QUERY_HEAD_FIELD_NAME, $arrayStrLen['name']), CGROUP_SHOW_QUERY_HEAD_FIELD_NAME).'</a></div></div>';
		$headers['name'] = '<div class="str"><a href="'.$href.'">'.altText(getCutString(CGROUP_SHOW_QUERY_HEAD_FIELD_NAME, $arrayStrLen['name']), CGROUP_SHOW_QUERY_HEAD_FIELD_NAME).'</a></div>';
	    }
	}
	...
	<div class="form query">
	    ...
	    <div class="top"></div>
	    <form name="formQueryGroup" id="formQueryGroup" method="post" action="">
	    ...
	    {
		?>
		<div class="message warning"><?php echo CGROUP_SHOW_QUERY_NOT_FOUND; ?></div>
		<?php
	    }
	    else
	    {
		...
		<div class="data">
		    ...
		    {
			?>
			<div class="row row<?php echo $class; ?>" id="group_tr_<?php echo $row['id']; ?>" data-table-name="group" data-id="<?php echo $row['id']; ?>" data-form-id="formQueryGroup">
			<?php
			if (is_array($groupActions) === TRUE)
			{
			    ?>
			    <div class="col header"></div>
			    <div class="col" style="width: <?php echo $widthGroupActions; ?>;"><div class="group-actions"><input name="groupGroup[]" type="checkbox" id="cb_group_<?php echo $row['id']; ?>" value="<?php echo $row['id']; ?>" onclick="checkboxClick(this)" /></div></div>
			    <?php
			}
	...
	if (is_array($groupActions) === TRUE)
	{
		...
		<div class="group-actions">
		    <input name="group_select_all" type="checkbox" id="group_select_all" value="" class="checkbox_show_query" onclick="querySelectAll('group', 'formQueryGroup')" />
		    <span><?php echo CGROUP_SHOW_QUERY_SELECT_ALL; ?></span>
		</div>
		<?php
		$j = 1;
		foreach ($groupActions as $value)
		{
		    if ($value['confirm'] === TRUE)
		    {
			$onclick = 'onclick="formQuerySubmit(\'formQueryGroup\', \''.$value['file'].'\', \''.$actionsParams.'\', true, \''.$value['msg'].'\');"';
		    }
		    else
		    {
			$onclick = 'onclick="formQuerySubmit(\'formQueryGroup\', \''.$value['file'].'\', \''.$actionsParams.'\', false, \'\');"';
		    }
		    if ($value['button'] === TRUE)
		    {
			echo '<input type="button" name="group_ga_'.$j.'" id="group_ga_'.$j.'" value="'.$value['text'].'" class="'.$value['class'].'" '.$onclick.' />';
		    }
		    else
		    {
			if ($value['image'] == '')
    ...
    /**
     * Muestra un listado de la tabla group en un campo select
     * 
     ...
     * @param string $filter [opcional] Filtro para el llenado del desplegable
     * @return string
     * @access public
     */
    ...
    /**
    * Devuelve si un grupo tiene permiso para un archivo
    *
    * Este método determina si el grupo del usuario administrador tiene permiso de acceso
    * al archivo indicado.
    *
    * @param integer $id ID del grupo
    * @param string $file archivo para controlar el permiso
    * @param boolean $boolean indica si el método devuelve un valor boolean o si hace una redirección en caso de que no tenga permiso
    * @return boolean
    */
    function filePermission($file, $boolean=FALSE)
    {
	$rs = FALSE;

	if (validateRequiredValue($this->getid(FALSE)) === TRUE and validateRequiredValue($file) === TRUE)
	{
	    $permission = new Cpermission();
	    $permission->setDbConn($this->getDbConn());

	    $res = $permission->getlist("name='".$file."'");

	    if ($res !== FALSE and is_array($res) === TRUE and count($res) > 0)
	    {
		$groupxpermission = new Cgroupxpermission();
		$groupxpermission->setDbConn($this->getDbConn());
		$groupxpermission->setid_group($this->getid());

		$groupxpermission->setid_permission($res[0]['id']);

		if ($groupxpermission->getdata() === TRUE)
		{
		    $rs = TRUE;
		}
	    }
